...
class tx_gridelements_drawitemhookTest extends \TYPO3\CMS\Extbase\Tests\Unit\BaseTestCase
{
    ...
    {
        $drawItemHook = $this->getMock('GridElementsTeam\\Gridelements\\Hooks\\DrawItem',
            array('renderCTypeGridelements', 'renderCTypeShortcut'));
        $drawItemHook->expects($this->never())->method('renderCTypeGridelements')->will($this->returnValue('gridelements'));
        $drawItemHook->expects($this->never())->method('renderCTypeShortcut')->will($this->returnValue('shortcut'));
    ...
    public function testPreProcessCTypeGridElements()
    {
        $drawItemHook = $this->getMock('GridElementsTeam\\Gridelements\\Hooks\\DrawItem',
            array('renderCTypeGridelements', 'renderCTypeShortcut'));
        $drawItemHook->expects($this->once())->method('renderCTypeGridelements')->will($this->returnValue('gridelements'));
        $drawItemHook->expects($this->never())->method('renderCTypeShortcut')->will($this->returnValue('shortcut'));
        ...
        $headerContent = 'header';
        $itemContent = '<li>Entry</li>';
        $row = array('uid' => 12, 'pid' => 13, 'CType' => 'gridelements_pi1');
        $drawItemHook->preProcess($parentObject, $drawItem, $headerContent, $itemContent, $row);
    ...
    public function testPreProcessCTypeShortcut()
    {
        $drawItemHook = $this->getMock('GridElementsTeam\\Gridelements\\Hooks\\DrawItem',
            array('renderCTypeGridelements', 'renderCTypeShortcut'));
        $drawItemHook->expects($this->never())->method('renderCTypeGridelements')->will($this->returnValue('gridelements'));
        $drawItemHook->expects($this->once())->method('renderCTypeShortcut')->will($this->returnValue('shortcut'));

        $parentObject = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('TYPO3\CMS\Backend\View\PageLayoutView');
        $drawItem = true;
        $headerContent = 'header';
        $itemContent = '<li>Entry</li>';
        $row = array('uid' => 12, 'pid' => 13, 'CType' => 'shortcut');
        $drawItemHook->preProcess($parentObject, $drawItem, $headerContent, $itemContent, $row);
    ...
    public function testSetMultipleColPosValues()
    {
        ...
        $drawItemHook->lang->expects($this->any())->method('sL')->will($this->onConsecutiveCalls('Überschrift', 'Text',
            'Bild'));
        $drawItemHook->setMultipleColPosValues($parserRows, $colPosValues);
        $this->assertEquals($expectedRows, $parserRows);
        ...
        $drawItemHook->lang->expects($this->any())->method('sL')->will($this->onConsecutiveCalls('Überschrift', 'Text',
            'Bild'));
        $drawItemHook->lang->expects($this->any())->method('getLL')->will($this->returnValue('Nicht zugewiesen'));
        $drawItemHook->setMultipleColPosValues($parserRows, $colPosValues);
        $this->assertEquals($expectedRows, $parserRows);
        $this->assertEquals($expectedColPosValues, $colPosValues);
        ...
        $drawItemHook->lang = $this->getMock('language');
        $drawItemHook->lang->expects($this->any())->method('sL')->will($this->onConsecutiveCalls('Überschrift', '',
            'Bild'));
        $drawItemHook->lang->expects($this->any())->method('getLL')->will($this->returnValue('Nicht zugewiesen'));
        $drawItemHook->setMultipleColPosValues($parserRows, $colPosValues);
        $this->assertEquals($expectedRows, $parserRows);
