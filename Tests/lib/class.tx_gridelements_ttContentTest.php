...
class tx_gridelements_ttContentTest extends \TYPO3\CMS\Extbase\Tests\Unit\BaseTestCase
{
    ...
    public function testColumnsItemsProcFunc()
    {
        ...
        $t3libDb->expects($this->once())->method('exec_SELECTgetSingleRow')->with($this->equalTo('*'),
            $this->equalTo('tt_content'),
            $this->equalTo('uid=123 AND tt_content.deleted=0'))->will($this->returnValue(array('tx_gridelements_backend_layout' => '2')));
        $GLOBALS['TYPO3_DB'] = $t3libDb;
        $layoutSetup = $this->getMock('GridElementsTeam\\Gridelements\\Backend\\LayoutSetup',
            array('getLayoutColumnsSelectItems'));
        ...
        $t3libDb->method('exec_SELECTgetSingleRow')->with($this->equalTo('*'), $this->equalTo('tt_content'),
            $this->equalTo('uid=123 AND tt_content.deleted=0'))->will($this->returnValue(array('tx_gridelements_backend_layout' => '2')));
        $GLOBALS['TYPO3_DB'] = $t3libDb;
