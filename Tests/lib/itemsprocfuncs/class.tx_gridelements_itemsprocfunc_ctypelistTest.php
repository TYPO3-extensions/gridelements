...
class tx_gridelements_itemsprocfunc_ctypelistTest extends \TYPO3\CMS\Extbase\Tests\Unit\BaseTestCase
{
    ...
    public function testItemsProcFunc()
    {
        ...
        $params['items'][2][1] = '2';
        $params['items'][2][2] = '';
        $colPosList = $this->getMock('GridElementsTeam\\Gridelements\\Backend\\ItemsProcFuncs\\CTypeList',
            array('checkForAllowedCTypes'));
        $colPosList->expects($this->any())->method('checkForAllowedCTypes')->with($this->equalTo($params['items']),
            $this->equalTo(12), $this->equalTo(2), $this->equalTo(23), $this->equalTo(34));
        ...
        $t3libDb->expects($this->once())->method('exec_SELECTgetSingleRow')->with($this->equalTo('pid, CType, colPos, tx_gridelements_container, tx_gridelements_columns'),
            $this->equalTo('tt_content'), $this->equalTo('uid=12'))->will($this->returnValue(null));
        $GLOBALS['TYPO3_DB'] = $t3libDb;
        $colPosList = $this->getMock('GridElementsTeam\\Gridelements\\Backend\\ItemsProcFuncs\\CTypeList',
            array('checkForAllowedCTypes'));
        $colPosList->expects($this->never())->method('checkForAllowedCTypes');
        $colPosList->itemsProcFunc($params);
        ...
        $t3libDb = $this->getMock('TYPO3\\CMS\\Core\\Database\\DatabaseConnection', array('exec_SELECTgetSingleRow'));
        $t3libDb->expects($this->once())->method('exec_SELECTgetSingleRow')->with($this->equalTo('pid, CType, colPos, tx_gridelements_container, tx_gridelements_columns'),
            $this->equalTo('tt_content'), $this->equalTo('uid=12'))->will($this->returnValue($returnValue));
        $GLOBALS['TYPO3_DB'] = $t3libDb;
        $colPosList = $this->getMock('GridElementsTeam\\Gridelements\\Backend\\ItemsProcFuncs\\CTypeList',
            array('checkForAllowedCTypes'));
        $colPosList->expects($this->once())->method('checkForAllowedCTypes')->with($this->equalTo($params['items']),
            $this->equalTo(123), $this->equalTo(20), $this->equalTo(45), $this->equalTo(67));
        $colPosList->itemsProcFunc($params);
    }
    ...
    {
        $cTypeList = $this->getMock('GridElementsTeam\\Gridelements\\Backend\\ItemsProcFuncs\\CTypeList',
            array('getSelectedBackendLayout'));
        ...
        $gridContainerId = 2;
        $gridColumn = 3;
        $cTypeList->expects($this->once())->method('getSelectedBackendLayout')->with($this->equalTo(12))->will($this->returnValue(array()));
        ...
        $layoutSetup->expects($this->once())->method('getLayoutSetup')->will($this->returnValue(array()));
        ...
        $layoutSetup = $this->getMock('GridElementsTeam\\Gridelements\\Backend\\LayoutSetup', array('getLayoutSetup'));
        $layoutSetup->expects($this->once())->method('getLayoutSetup')->will($this->returnValue($returnValue));
        $cTypeList->injectLayoutSetup($layoutSetup);
