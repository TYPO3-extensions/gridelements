...
class tx_gridelements_itemsprocfunc_colposlistTest extends \TYPO3\CMS\Extbase\Tests\Unit\BaseTestCase {
	...
	public function testItemsProcFunc() {
		...
		$colPosList->expects($this->once())->method('addColPosListLayoutItems')->with($this->equalTo(12), $this->equalTo($params['items']), $this->equalTo('gridelements'))->will($this->returnValue('Hello world'));
		$testParams = $params;
		...
		$t3libDb->expects($this->once())->method('exec_SELECTgetSingleRow')->with($this->equalTo('pid, CType'), $this->equalTo('tt_content'), $this->equalTo('uid=15'))->will($this->returnValue($dbReturnValue));
