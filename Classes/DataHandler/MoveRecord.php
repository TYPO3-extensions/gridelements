...
class MoveRecord extends AbstractDataHandler
{
     ...
     * @return void
     */
    public function execute_moveRecord(
        $table,
        $uid,
        &$destinationPid,
        &$propertyArray,
        ...
        if ($table === 'tt_content' && !$this->getTceMain()->isImporting) {
            ...
            if (strpos($cmd['tt_content'][$uid]['move'], 'x') !== false) {
                $targetAvailable = $this->updateTargetContainerAndResolveTargetId($cmd, $uid, $destinationPid,
     ...
     */
    public function execute_moveRecord_afterAnotherElementPostProcess(
        $table,
        $uid,
        $destinationPid,
        ...
        $targetElement = BackendUtility::getRecordWSOL('tt_content', -$originalDestinationPid,
            'uid,pid,colPos,tx_gridelements_container,tx_gridelements_columns');
        ...
        if (strpos($pasteUid, 'x') !== false) {
            ...
            $GLOBALS['TCA']['tt_content']['ctrl']['copyAfterDuplFields'] = str_replace('colPos,', '',
     ...
     */
    public function createUpdateArrayForSplitElements(
        $recordUid,
        &$destinationPid,
        $targetUid,
        array $target,
        array &$containerUpdateArray
    ) {
