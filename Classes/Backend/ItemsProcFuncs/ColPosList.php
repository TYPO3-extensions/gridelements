...
class ColPosList extends AbstractItemsProcFunc {
	...
	public function itemsProcFunc(&$params) {
		...
		if ($params['row']['pid'] > 0) {
			...
			$existingElement = $this->databaseConnection->exec_SELECTgetSingleRow('pid, CType', 'tt_content', 'uid=' . -((int)$params['row']['pid']));
	...
	protected function addColPosListLayoutItems($pageId, array $items, $CType = '') {
		...
		$items[] = array($this->lang->sL('LLL:EXT:gridelements/Resources/Private/Language/locallang_db.xml:tt_content.tx_gridelements_container'), '-1', null, null);
