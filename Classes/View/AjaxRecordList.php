...
class AjaxRecordList
{
    ...
    public function getListRows(AjaxRequestHandler &$ajaxObj)
    {
        ...
        if ($uid > 0) {
            ...
            $elementChildren = Helper::getInstance()->getChildren($table, $uid, GeneralUtility::_GP('sortField'),
                (int)GeneralUtility::_GP('sortRev'));
    ...
    public function getRecordList($table, $uid, $row)
    {
        ...
        if ($access) {
            ...
            $dataBaseList->allowedNewTables = GeneralUtility::trimExplode(',',
                $modTSconfig['properties']['allowedNewTables'], 1);
            $dataBaseList->deniedNewTables = GeneralUtility::trimExplode(',',
                $modTSconfig['properties']['deniedNewTables'], 1);
            ...
            if ($this->cmd == 'setCB') {
                ...
                $CB['el'] = $dataBaseList->clipObj->cleanUpCBC(array_merge((array)GeneralUtility::_POST('CBH'),
                    (array)GeneralUtility::_POST('CBC')), $cmd_table);
            }
